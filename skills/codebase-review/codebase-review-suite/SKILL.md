...
1. `codebase-consolidation-cleanup` for a non-modifying assessment of unused, duplicate, and overlapping code paths.
...
- `codebase-decomposition` (user-invoked): plan or execute an accepted module-splitting/decomposition finding from `codebase-architecture-review`. Tell the user to run `/codebase-decomposition`; this skill cannot start it.
- `codebase-cleanup` (user-invoked): actually delete the dead code, stale scripts, and unused dependencies that the assessment identified. Tell the user to run `/codebase-cleanup`; this skill cannot start it.
- `optimize-prompt-caching`: audit or implement accepted LLM prompt caching cost/latency findings from `codebase-performance-review` or LLM call-site investigations. Call the Skill tool with "optimize-prompt-caching".

Do not run follow-up skills as default suite lenses. They are implementation workflows, not assessment lenses. This suite never deletes or rewrites code: every destructive follow-up is user-invoked by design, so a human has to start it.
