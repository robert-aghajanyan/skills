...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/update_manifest.py" "${CLAUDE_SKILL_DIR}"
```
...
```bash
python3 "${CLAUDE_SKILL_DIR}/../../meta/skill-builder/scripts/validate-skill.py" "${CLAUDE_SKILL_DIR}"
```

Run the semantic smoke tests after meaningful edits:

```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/smoke_production_readiness_gate.py" "${CLAUDE_SKILL_DIR}"
```
